...
import "./interfaces/IMerkleProofWrapper.sol";

contract MerkleProofWrapper is IMerkleProofWrapper {
    function verify(
    ...
    function processProof(bytes32[] memory proof, bytes32 leaf) external pure override returns (bytes32) {
        return MerkleProof.processProof(proof, leaf);
    }

    function processProofCalldata(bytes32[] calldata proof, bytes32 leaf) external pure override returns (bytes32) {
